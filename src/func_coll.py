...
def place_record_into_db(playlist, duration, path):
    """
...
def remove_record_db(row_min, row_max):
    removed_tracks_amount = row_max + 1 - row_min
    cur.execute("DELETE FROM {0} WHERE row_id BETWEEN ? AND ?"
                .format(cv.active_db_table),(row_min + 1, row_max + 1,))
    cur.execute("UPDATE {0} SET row_id = row_id - ? WHERE row_id > ?"
                .format(cv.active_db_table),(removed_tracks_amount, row_min + 1,))
    connection.commit()
...
def get_duration_db_sum_multi_selection(row_min, row_max):
    return cur.execute("SELECT SUM(duration) FROM {0} WHERE row_id BETWEEN ? AND ?"
                     .format(cv.active_db_table),(row_min + 1, row_max + 1,)).fetchall()[0][0]
...
def add_media_grouped_actions(playlist, track_path, raw_duration):
    # The DB will be saved in "thread_add_media / return_thread_generated_values()"
    # at the end of "add media" process
    place_record_into_db(playlist, raw_duration, track_path)

    track_name = Path(track_path).stem
    row_id = cv.playlist_widget_dic[playlist]['name_list_widget'].count() + 1
    new_track_name = f'{row_id}. {track_name}'
    duration = generate_duration_to_display(raw_duration)

    add_new_list_item(new_track_name, cv.playlist_widget_dic[playlist]['name_list_widget'], None, track_path)
    add_new_list_item('',  cv.playlist_widget_dic[playlist]['queue_list_widget'])
    add_new_list_item(duration,  cv.playlist_widget_dic[playlist]['duration_list_widget'])
...
def update_queued_tracks_after_track_deletion(row_min, row_max):
    if cv.active_db_table in cv.queue_playlists_list:
        for removed_row_index in reversed(range(row_min, row_max+1)):
            cv.queue_tracking_title = [cv.active_db_table, removed_row_index]
...
def remove_track_from_playlist():
    ...
    if selected_row_number_list:
        ...
        # QUEUE LIST UPDATE
        update_queued_tracks_after_track_deletion(row_min, row_max)
...
def set_vertical_scrollbar_style_default(scrollbar):
    """
    Used for the Settings window, Queue & Search window and Thumbnail view
    """
